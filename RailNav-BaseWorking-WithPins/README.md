...
## Backend API Documentation (NEW)

The system now uses a Flask backend that powers key data features:

### Endpoints

- `GET /api/crowd`
  - Returns: `{ status: 'success', density: 'Low'|'Medium'|'High' }`
  - Used by dashboard to render live crowd bar and density label.
- `GET /api/navigation`
  - Returns: `{ status: 'success', route: [{lat, lng}, ...] }`
  - Consumed by navigation/AR module to render indoor paths or demo routes.

### Example Usage

Fetch crowd density (JS or cURL):

```js
fetch("http://127.0.0.1:5000/api/crowd")
  .then((r) => r.json())
  .then(console.log);
```

```bash
curl http://127.0.0.1:5000/api/crowd
```
...
- **Front end:** Modularized HTML/JS/CSS (no frameworks; in-place improvements)
  - `dashboard.html`—fetches and displays crowd data using `/api/crowd` endpoint
  - `gptmpa1.html`—navigation and AR routing using `/api/navigation` path from backend
- **Backend:** Flask, extensible for real ML, station maps, admin uploads, more
  - Simple data exchange: plain JSON over REST
  - Extensible: for real-time or ML-powered crowd data, station importers, admin tools (future)

---

## Extending for ML & Admin Features

- Plug in ML/camera/IoT for real crowd estimation in `/api/crowd`.
- Expand `/api/navigation` to support admin-uploaded station maps, dynamic user/destination params, and pathfinding algorithms for new stations.

---

For developers: run backend (`python backend/app.py`), then open dashboard.html and navigation views in browser. All API endpoints are documented/in-code and ready for integration/extension.
